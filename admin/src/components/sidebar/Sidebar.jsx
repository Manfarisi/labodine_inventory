...
const navItems = [
  ...
  },
  {
    ...
    icon: Package,
    submenu: [
      { title: "Daftar Produk", href: "/list",allowedRoles: ["Admin", "Pegawai"] },
      { title: "Daftar Bahan", href: "/daftarBaku",allowedRoles: ["Admin", "Pegawai"] },
    ],
  },

  {
    ...
    icon: ShoppingBasket,
    submenu: [
      { title: "Pemasukan", href: "/daftarPemasukan", allowedRoles: ["Admin", "Pegawai"]},
      { title: "Pengeluaran", href: "/daftarPengeluaran",allowedRoles: ["Admin", "Pegawai"] },
    ],
  },
   {
    title: "Absensi",
...
const userRole = user?.kategori || "pegawai"; // default fallback
...
<div className="flex flex-col min-h-screen bg-gray-800 text-white w-64 shadow-lg">
  ...
  <div className="flex-1 overflow-y-auto p-4">
    <nav className="space-y-2">
      {navItems.map((item) => {
        const isParentActive = isSubLinkActive(item.submenu) || isActiveLink(item.href);
        return (
          <div key={item.href}>
            {item.submenu ? (
              <div>
                <button
                  ...
                  className={`flex items-center w-full p-2 rounded-md transition-colors duration-200
                    ${isParentActive ? 'bg-gray-700 text-white' : 'hover:bg-gray-700 text-gray-300'}`}
                >
                  <item.icon className="h-5 w-5 mr-3" />
                  <span>{item.title}</span>
                  <ChevronDown
                    className={`ml-auto h-4 w-4 transition-transform ${openSubmenus[item.href] || isSubLinkActive(item.submenu) ? 'rotate-180' : ''}`}
                  />
                </button>
{(openSubmenus[item.href] || isSubLinkActive(item.submenu)) && (
  <div className="ml-6 mt-1 space-y-1">
    {item.submenu
      .filter((subItem) => subItem.allowedRoles?.includes(userRole)) // <-- filter berdasarkan role
      .map((subItem) => (
        <Link
          key={subItem.href}
          to={subItem.href}
